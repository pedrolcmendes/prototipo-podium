...
export default function Nav() {
  ...
  useEffect(() => { fetchPendingPayment(); }, [user]);
  useLive(['bookings'], fetchPendingPayment);

  useEffect(() => {
    const handler = () => setHasPendingPayment(true);
    window.addEventListener('podium:booking-pending', handler);
    return () => window.removeEventListener('podium:booking-pending', handler);
  }, []);
    ...
    <>
      ...
      >
        ...
        <div className="mm-group">
          ...
          <Link className={`mm-item${location.pathname === '/campeonatos' ? ' active' : ''}`} to="/campeonatos" onClick={() => setMenuOpen(false)}>
              ...
              <>
                ...
                {user.admin ? (
                  ...
                  <>
                    ...
                    {hasPendingPayment && (
                      <Link className="mm-btn" style={{ color: 'var(--gold)' }} to="/painel?tab=reservas&filter=pendentes" onClick={() => setMenuOpen(false)}>
      ...
      <nav id="mainNav" className={`${scrolled ? 'scrolled' : ''}${menuOpen ? ' menu-open' : ''}`}>
        ...
        <ul className="nav-links">
          ...
          <li><Link to="/campeonatos" className={location.pathname === '/campeonatos' ? 'active' : ''}>Campeonatos</Link></li>
          ...
          {user ? (
            <div className="user-avatar-pill" ref={pillRef} onClick={() => setDdOpen(o => !o)}>
              ...
              <div className={`user-dropdown${ddOpen ? ' open' : ''}`}>
                ...
                {hasPendingPayment && (
                  <Link to="/painel?tab=reservas&filter=pendentes" style={{ color: 'var(--gold)' }}>
