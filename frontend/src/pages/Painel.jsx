...
export default function Painel() {
  ...
  const hasPassword = user?.hasPassword ?? !user?.googleId;
  ...
  const handleSaveSenha = async (e) => {
    e.preventDefault();
    if (hasPassword && !senhaData.senhaAtual) { toast('Informe a senha atual', 'error'); return; }
    ...
    try {
      const { data } = await api.put('/users/me/password', {
        ...(hasPassword ? { senhaAtual: senhaData.senhaAtual } : {}),
        novaSenha: senhaData.novaSenha,
      });
      updateUser(data.user || { hasPassword: true });
      toast(data.message || (hasPassword ? 'Senha alterada!' : 'Senha configurada!'), 'success');
        ...
        <main className="painel-content">
          ...
          {tab === 'perfil' && (
            <section className="painel-section active">
              ...
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 0, padding: '1.5rem 1.8rem', display: 'flex', alignItems: 'center', gap: '1.4rem', marginBottom: '1.2rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', marginTop: '.55rem', flexWrap: 'wrap' }}>
                    ...
                    {user?.googleId && (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '.35rem', fontFamily: 'var(--font-cond)', fontSize: '.68rem', letterSpacing: '1.5px', color: 'var(--gray)', background: 'var(--dark)', border: '1px solid var(--border)', borderRadius: 0, padding: '.2rem .6rem' }}>
              ...
              <form className="profile-form" style={{ borderRadius: 0, overflow: 'hidden', marginBottom: '1.2rem' }} onSubmit={handleSavePerfil}>
              ...
              <form className="profile-form" style={{ borderRadius: 0, overflow: 'hidden' }} onSubmit={handleSaveSenha}>
                <div className="profile-form-section">
                  <h3>{hasPassword ? 'Alterar Senha' : 'Criar Senha de Acesso'}</h3>
                  {!hasPassword && (
                    <p style={{ marginTop: '.65rem', color: 'var(--gray)', fontSize: '.78rem' }}>
                      Sua conta foi criada pelo Google. Crie uma senha para também entrar usando seu e-mail.
                    </p>
                  )}
                  <div className="profile-grid" style={{ marginTop: '1.2rem' }}>
                    {hasPassword && (
                      <div className="field" style={{ gridColumn: '1 / -1' }}>
                        ...
                        <input type="password" autoComplete="current-password" placeholder="••••••••" value={senhaData.senhaAtual} onChange={e => setSenhaData({ ...senhaData, senhaAtual: e.target.value })} />
                      </div>
                    )}
                    <div className="field">
                      <label>{hasPassword ? 'Nova senha' : 'Crie uma senha'}</label>
                      <input type="password" autoComplete="new-password" placeholder="Mín. 6 caracteres" value={senhaData.novaSenha} onChange={e => setSenhaData({ ...senhaData, novaSenha: e.target.value })} />
                    </div>
                    <div className="field">
                      <label>Confirmar nova senha</label>
                      <input type="password" autoComplete="new-password" placeholder="Repetir senha" value={senhaData.confirmar} onChange={e => setSenhaData({ ...senhaData, confirmar: e.target.value })} />
                    </div>
                  </div>
                </div>
                <div className="profile-save" style={{ padding: '1rem 1.8rem', borderTop: '1px solid var(--border)' }}>
                  <span style={{ fontSize: '.75rem', color: 'var(--gray)' }}>
                    {hasPassword ? 'Sua sessão continuará ativa após a alteração.' : 'Depois disso, Google e senha funcionarão no acesso.'}
                  </span>
